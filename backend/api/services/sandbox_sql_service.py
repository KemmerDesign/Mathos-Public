"""
Mathós — Sandbox SQL para certificaciones de base de datos.

Ejecuta SQL contra una BD SQLite pre-poblada con el esquema HR de Oracle.
Para SELECT: ejecución directa. Para DML/DDL: copia en memoria para no corromper datos base.
Añade análisis de IA sobre la consulta.
"""
...
# Ruta de la BD sandbox
DATA_DIR = Path(__file__).resolve().parent.parent / "data"
DATA_DIR.mkdir(parents=True, exist_ok=True)
SANDBOX_DB = str(DATA_DIR / "oracle_sandbox.db")

# ────────────────────────────────────────────────────────────────────────────
# Esquema HR de Oracle (SQLite-compatible)
# ────────────────────────────────────────────────────────────────────────────
...
-- Tabla DUAL de Oracle (compatibilidad)
...
('BR','Brazil',2),('CA','Canada',2),('CN','China',3),
('DE','Germany',1),('EG','Egypt',4),('FR','France',1),
('IL','Israel',4),('IN','India',3),('IT','Italy',1),
('JP','Japan',3),('KW','Kuwait',4),('ML','Malaysia',3),
('MX','Mexico',2),('NG','Nigeria',4),('NL','Netherlands',1),
('SG','Singapore',3),('UK','United Kingdom',1),('US','United States of America',2),
('ZA','South Africa',4),('ZM','Zambia',4),('ZW','Zimbabwe',4),('CH','Switzerland',1);
...
(108,'Nancy','Greenberg','NGREENBE','515.124.4569','1994-08-17','FI_MGR',12008,NULL,101,100),
...
(126,'Irene','Mikkileni','IMIKKILE','650.124.1224','1998-09-28','ST_CLERK',2700,NULL,120,50),
...
(168,'Lisa','Ozer','LOZER','011.44.1343.629268','1997-03-11','SA_REP',11500,0.25,148,80),
...
(205,'Shelley','Higgins','SHIGGINS','515.123.8080','1994-06-07','AC_MGR',12008,NULL,101,110),
...
INSERT OR IGNORE INTO dual VALUES ('X');
"""
...
def _classify_sql(sql: str) -> str:
    """Clasifica el tipo de sentencia SQL."""
    ...
    if cleaned.startswith("MERGE"):
        return "MERGE"
    if any(cleaned.startswith(k) for k in ("CREATE", "ALTER", "DROP", "TRUNCATE")):
        return "DDL"
    if cleaned.startswith("BEGIN") or cleaned.startswith("DECLARE"):
        return "PLSQL"
    return "OTHER"


def _format_rows(cursor, rows: list) -> dict:
    """Convierte filas de cursor a formato serializable."""
    if cursor.description is None:
        return {"columns": [], "rows": []}
    columns = [d[0] for d in cursor.description]
    return {
        "columns": columns,
        "rows": [list(row) for row in rows[:500]],  # max 500 filas
    }


def ejecutar_sql(sql: str) -> dict:
    """
    Ejecuta SQL contra el sandbox SQLite.

    Para SELECT: ejecuta directamente.
    Para DML: ejecuta en copia en memoria, no persiste cambios.
    Para DDL/PL/SQL: devuelve error explicativo.
    """
    _ensure_sandbox()
    ...
    if sql_type == "PLSQL":
        return {
            ...
            "success": False,
            "error": (
                "El sandbox usa SQLite — PL/SQL no se puede ejecutar directamente. "
                "Escribe el bloque PL/SQL y usa el botón 'Analizar con IA' para recibir "
                "feedback sobre la sintaxis y lógica."
            ),
            "columns": [],
            "rows": [],
            "rows_affected": 0,
    ...
    if sql_type == "DDL":
        # Ejecutar DDL en memoria para que no afecte la BD base
        try:
            conn = sqlite3.connect(":memory:")
            # Cargar el esquema HR en memoria
            base_conn = sqlite3.connect(SANDBOX_DB)
            for line in base_conn.iterdump():
                conn.execute(line) if line != "BEGIN;" and line != "COMMIT;" else None
            base_conn.close()
            conn.commit()
            conn.execute(sql)
            conn.commit()
            ...
            return {
                "tipo": sql_type,
                "success": True,
                "mensaje": "DDL ejecutado en sandbox temporal (no persiste — el esquema HR base no se modifica).",
                "columns": [],
                "rows": [],
                "rows_affected": 0,
            }
    ...
    # DML — ejecutar en copia en memoria
    try:
        base_conn = sqlite3.connect(SANDBOX_DB)
        ...
        return {
            "tipo": sql_type,
            "success": True,
            "mensaje": f"{rows_affected} fila(s) afectada(s). (Cambios en sandbox temporal — no persisten).",
            "columns": [],
            "rows": [],
...
ANALISIS_SQL_PROMPT = """Eres un instructor de certificaciones Oracle Database Administrator 19c (OCA/OCP).
El estudiante ha escrito el siguiente SQL en un sandbox con el esquema HR de Oracle (employees, departments, jobs, locations, etc.).
...
Contexto: Tema "{tema_nombre}" — Materia "Oracle Database Administrator 19c"

Analiza el SQL y proporciona feedback educativo en 4 partes:
1. **Corrección**: ¿Es sintácticamente correcto? ¿Producirá el resultado esperado?
2. **Equivalente Oracle 19c**: ¿Hay funciones/cláusulas Oracle específicas que no existen en SQLite? Muestra el equivalente Oracle real si aplica.
3. **Buenas prácticas**: ¿Sigue las convenciones Oracle? (aliases, indentación, uso de ANSI joins, etc.)
4. **Sugerencias de mejora**: ¿Cómo se puede optimizar o hacer más idiomático?

Si el SQL es para el examen 1Z0-082, menciona conceptos relacionados que pueden aparecer en el examen.
Usa formato Markdown. Sé conciso pero completo."""


async def analizar_sql_con_ia(
    sql: str,
    resultado_ejecucion: dict,
    tema_nombre: str = "",
) -> str:
    """Llama a la IA para analizar el SQL y dar feedback educativo."""
    resultado_str = json.dumps(resultado_ejecucion, ensure_ascii=False, indent=2)
    if len(resultado_str) > 800:
        resultado_str = resultado_str[:800] + "...[truncado]"

    prompt = ANALISIS_SQL_PROMPT.format(
        sql=sql,
        resultado_ejecucion=resultado_str,
        tema_nombre=tema_nombre or "SQL Oracle",
    )
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                resp = await client.post(
                    ...
                    json={
                        "model": model,
                        "messages": [
                            {"role": "system", "content": "Eres un instructor experto en certificaciones Oracle Database. Responde en español."},
    ...
    return "No se pudo obtener análisis de IA en este momento. Revisa tu código manualmente y verifica la sintaxis Oracle."


def get_schema_info() -> dict:
    """Retorna información del esquema disponible en el sandbox."""
    return {
        "tablas": [
            {"nombre": "employees", "columnas": "employee_id, first_name, last_name, email, phone_number, hire_date, job_id, salary, commission_pct, manager_id, department_id", "filas": 107},
            {"nombre": "departments", "columnas": "department_id, department_name, manager_id, location_id", "filas": 27},
            {"nombre": "jobs", "columnas": "job_id, job_title, min_salary, max_salary", "filas": 19},
            {"nombre": "locations", "columnas": "location_id, street_address, postal_code, city, state_province, country_id", "filas": 23},
            {"nombre": "countries", "columnas": "country_id, country_name, region_id", "filas": 25},
            {"nombre": "regions", "columnas": "region_id, region_name", "filas": 4},
            {"nombre": "job_history", "columnas": "employee_id, start_date, end_date, job_id, department_id", "filas": 10},
            {"nombre": "dual", "columnas": "dummy", "filas": 1},
        ],
        "nota": "Esquema HR (Human Resources) de Oracle — el más usado en certificaciones OCA/OCP. SQLite ejecuta SQL estándar; funciones Oracle-específicas (DECODE, NVL, etc.) pueden no estar disponibles — usa el análisis IA para ver el equivalente real.",
    }
